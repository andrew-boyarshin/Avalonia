    ...
    internal class Direct2DImageSurface : IExternalDirect2DRenderTargetSurface, IDisposable
    {
        class SwapBuffer : IDisposable
        {
            ...
            public SwapBuffer(IntSize size, Vector dpi)
            {
                ...
                _event = s_dxDevice.CreateQuery(new QueryDescription(QueryType.Event));
                ...
                using (var resource = texture.QueryInterface<IDXGIResource>())
                {
                    ...
                    using (var texture9 = s_d3DDevice.CreateTexture(
                        ...
                        Usage.RenderTarget, Vortice.Direct3D9.Format.A8R8G8B8, Pool.Default, ref handle))
        ...
        private void EnsureDirectX()
        {
            ...
            Vortice.Direct3D9.PresentParameters presentparams = new Vortice.Direct3D9.PresentParameters
            {
                Windowed = true,
