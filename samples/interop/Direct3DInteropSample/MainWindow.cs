    ...
    public class MainWindow : Window
    {
        ...
        private void CreateMesh()
        {
            ...
            var vertexBuffer = device.CreateBuffer(
                vertices,
                new BufferDescription(Unsafe.SizeOf<Vector4>() * vertices.Length, BindFlags.VertexBuffer, ResourceUsage.Default)
                );

            // Create Constant Buffer
            _contantBuffer = device.CreateBuffer(new BufferDescription(Unsafe.SizeOf<Matrix>(), BindFlags.ConstantBuffer, ResourceUsage.Default));
